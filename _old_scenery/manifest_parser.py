...
from app.tests.utils import read_yaml, SingleKeyDict

from app.tests.views.dataclasses import Manifest, Substituable  # , CommonCase
...
class ManifestParser:
    ...
    common_items = read_yaml("app/tests/views/common_items.yml")
    ...
    def parse_formatted_dict(d: dict):
        ...
        d = {key: d[key.value] for key in ManifestFormattedDictKeys}
        return Manifest.from_formatted_dict(d)
    ...
    def validate_dict(d: dict):
        ...
        if not all(key in [x.value for x in ManifestDictKeys] for key in d.keys()):
            raise ValueError(
    ...
    def validate_yaml(yaml):
        ...
        if not all(key in [x.value for x in ManifestYAMLKeys] for key in yaml.keys()):
            raise ValueError(
