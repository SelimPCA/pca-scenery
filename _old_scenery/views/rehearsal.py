from http import HTTPMethod
import unittest

from django.apps import apps
from django.http import HttpResponse, HttpResponseRedirect


# Scenery
from app.tests.utils import SingleKeyDict, colorize
from app.tests.views.dataclasses import (
    SetUpInstruction,
    Item,
    Case,
    HttpDirective,
    HttpScene,
    Manifest,
    HttpCheck,
    HttpTake,
    Substituable,
)
from app.tests.views.enums import (
    SetUpCommand,
    DirectiveCommand,
    DomArgument,
    # HttpTarget,
    ManifestFormattedDictKeys,
)
from app.tests.views.manifest_parser import ManifestParser
from app.tests.views.method_builder import MethodBuilder
from app.tests.views.set_up_handler import SetUpHandler
from app.tests.views.checker import HttpChecker

# from app.tests.views.take_builder import TakeBuilder
from app.tests.views.unittest import TestCaseOfDjangoTestCase

# from app.tests.views import logger_rehearsal

# Models
from app.models.accounts.profile import Profile, School
from app.models.accounts.user_custom import UserCustom
from app.models.v1.user_interactions import UserInteraction, UserInteractionDetail
from app.models.subscriptions.stripe_customer import StripeCustomer
from app.models.subscriptions.stripe_subscription import StripeSubscription


############
# DATCLASSES
############


class TestSingleKeyDict(unittest.TestCase):

    def test(self):
        d = SingleKeyDict({"key": "value"})
        self.assertEqual(d.key, "key")
        self.assertEqual(d.value, "value")
        self.assertEqual(d.as_tuple(), ("key", "value"))
        with self.assertRaisesRegex(
            ValueError, r"^SingleKeyDict should have length 1 not '2'"
        ):
            d = SingleKeyDict({"1": None, "2": None})


class TestSetUpInstruction(unittest.TestCase):

    def test(self):
        # params
        cmd_name = "reset_db"
        cmd = SetUpCommand.RESET_DB
        args = {"arg": object()}

        # __init__
        instruction = SetUpInstruction(cmd)
        self.assertEqual(instruction.command, cmd)
        self.assertEqual(instruction.args, {})

        instruction = SetUpInstruction(cmd, args)
        self.assertEqual(instruction.command, cmd)
        self.assertEqual(instruction.args, args)

        # from_object
        instruction = SetUpInstruction.from_object(cmd_name)
        self.assertEqual(instruction, SetUpInstruction(cmd))
        instruction = SetUpInstruction.from_object({cmd_name: args})
        self.assertEqual(instruction, SetUpInstruction(cmd, args))


class TestItem(unittest.TestCase):

    def test(self):
        d = {"a": object()}
        item = Item("id", d)
        self.assertEqual(item._id, "id")
        self.assertEqual(item["a"], d["a"])


class TestCase(unittest.TestCase):

    # TODO: wahou this is a very bad name

    def test(self):
        case_a = Case("id", {"item_id": Item("item_id", {})})
        self.assertEqual(case_a._id, "id")
        self.assertEqual(case_a.items, {"item_id": Item("item_id", {})})
        case_b = Case.from_id_and_dict("id", {"item_id": {}})
        self.assertEqual(case_a, case_b)


class TestHttpDirective(unittest.TestCase):

    def test(self):
        HttpDirective(DirectiveCommand("status_code"), 200)
        HttpDirective(DirectiveCommand("redirect_url"), "https://www.example.com")
        HttpDirective(DirectiveCommand("dom_element"), {"find": object()})
        HttpDirective(DirectiveCommand("count_instances"), {"model": "Profile", "n": 1})
        with self.assertRaises(ValueError):
            HttpDirective(DirectiveCommand("status_code"), "200")
        with self.assertRaises(ValueError):
            HttpDirective(DirectiveCommand("redirect_url"), 0)
        with self.assertRaises(ValueError):
            HttpDirective(DirectiveCommand("dom_element"), 0)
        with self.assertRaises(LookupError):
            HttpDirective(
                DirectiveCommand("count_instances"), {"model": "NotAModel", "n": 1}
            )

        HttpDirective.from_dict({"dom_element": {"find": object()}})
        HttpDirective.from_dict({"dom_element": {"find": object(), "scope": {}}})
        HttpDirective.from_dict({"dom_element": {"find_all": object()}})
        with self.assertRaises(ValueError):
            HttpDirective.from_dict({"dom_element": {"scope": {}}})
        with self.assertRaises(ValueError):
            HttpDirective.from_dict(
                {"dom_element": {"find": object(), "find_all": object()}}
            )


class TestSubstituable(unittest.TestCase):

    def test(self):
        case = Case("id", {"item_id": Item("item_id", {"a": object()})})
        sub = Substituable("item_id")
        x = sub.shoot(case)
        self.assertDictEqual(x, case["item_id"]._dict)
        sub = Substituable("item_id:a")
        x = sub.shoot(case)
        self.assertEqual(x, case["item_id"]["a"])

    def test_regex(self):

        self.assertRegex("item", Substituable.regex_field)
        self.assertRegex("item_with_underscore", Substituable.regex_field)
        self.assertRegex("item:field", Substituable.regex_field)
        self.assertRegex("item:field_with_underscore", Substituable.regex_field)
        self.assertNotRegex("item_Uppercase", Substituable.regex_field)
        self.assertNotRegex("item_0", Substituable.regex_field)
        self.assertNotRegex("item-with-hyphen", Substituable.regex_field)
        self.assertNotRegex("item:field_Uppercase", Substituable.regex_field)
        self.assertNotRegex("item:field_0", Substituable.regex_field)
        self.assertNotRegex("item:field-with-hyphen", Substituable.regex_field)


class TestHttpScene(unittest.TestCase):

    def setUp(self):
        super().setUp()
        self.scene_base_dict = {
            "method": "GET",
            "url": "https://www.example.com",
            "directives": [{"status_code": 200}],
        }

        case_dict = {
            "item_id": {
                "a": object(),
                "b": object(),
                "status_code": 200,
                "dom_element_id": "id",
                "attribute_value": "value",
            },
        }

        self.case = Case.from_id_and_dict("case_id", case_dict)

    def test(self):
        HttpScene(
            "GET",
            "https://www.example.com",
            [HttpDirective(DirectiveCommand("status_code"), 200)],
            [],
            {},
            {},
        )
        HttpScene.from_dict(
            {
                "method": "GET",
                "url": "https://www.example.com",
                "directives": [{"status_code": 200}],
                "data": [],
                "url_parameters": {},
                "query_parameters": {},
            }
        )

    def test_substitute_recusively(self):

        scene = HttpScene.from_dict(
            self.scene_base_dict
            | {
                "data": Substituable("item_id"),
                "url_parameters": Substituable("item_id"),
                "directives": [
                    {DirectiveCommand.STATUS_CODE: Substituable("item_id:status_code")}
                ],
            }
        )
        data = HttpScene.substitute_recursively(scene.data, self.case)
        self.assertDictEqual(data, self.case["item_id"]._dict)
        url_parameters = HttpScene.substitute_recursively(
            scene.url_parameters, self.case
        )
        self.assertDictEqual(url_parameters, self.case["item_id"]._dict)
        checks = HttpScene.substitute_recursively(scene.directives, self.case)

        self.assertEqual(
            checks[0],
            HttpCheck.from_dict({"status_code": self.case["item_id"]["status_code"]}),
        )

    def test_shoot(self):
        scene = HttpScene.from_dict(
            self.scene_base_dict
            | {"data": {"a": Substituable("item_id:a")}}
            | {"url_parameters": {"key": Substituable("item_id:a")}}
            | {
                "directives": [
                    {
                        "dom_element": {
                            "find": {"id": Substituable("item_id:dom_element_id")},
                            "attribute": {
                                "name": "name",
                                "value": Substituable("item_id:attribute_value"),
                            },
                        }
                    }
                ]
            }
        )
        take = scene.shoot(self.case)
        self.assertEqual(
            take,
            HttpTake(
                method=self.scene_base_dict["method"],
                url=self.scene_base_dict["url"],
                data={"a": self.case["item_id"]["a"]},
                url_parameters={"key": self.case["item_id"]["a"]},
                query_parameters={},
                checks=[
                    HttpCheck(
                        instruction=DirectiveCommand.DOM_ELEMENT,
                        args={
                            "find": {
                                "id": self.case["item_id"]["dom_element_id"],
                            },
                            "attribute": {
                                "name": "name",
                                "value": self.case["item_id"]["attribute_value"],
                            },
                        },
                    )
                ],
            ),
        )


class TestManifest(unittest.TestCase):

    def test(self):
        scene = HttpScene(
            "GET",
            "https://www.example.com",
            [HttpDirective(DirectiveCommand("status_code"), 200)],
            [],
            {},
            {},
        )
        scenes = [scene]
        set_up_test_data = [SetUpInstruction(SetUpCommand.RESET_DB)]
        set_up = [SetUpInstruction(SetUpCommand.LOGIN)]
        cases = [Case("a", [Item("item_id", {})]), Case("b", [Item("item_id", {})])]

        Manifest(set_up_test_data, set_up, scenes, cases, "origin")
        Manifest.from_formatted_dict(
            {
                ManifestFormattedDictKeys.SET_UP_TEST_DATA: ["reset_db"],
                ManifestFormattedDictKeys.SET_UP: ["login"],
                ManifestFormattedDictKeys.CASES: {"case_id": {"item_id": {}}},
                ManifestFormattedDictKeys.SCENES: [
                    {
                        "method": "GET",
                        "url": "https://www.example.com",
                        "data": [],
                        "url_parameters": {},
                        "query_parameters": {},
                        "directives": [{"status_code": 200}],
                    }
                ],
                ManifestFormattedDictKeys.MANIFEST_ORIGIN: "origin",
            }
        )


class TestHttpCheck(unittest.TestCase):

    def test(self):

        class NotAModel:
            pass

        HttpCheck(DirectiveCommand("status_code"), 200)
        HttpCheck(DirectiveCommand("redirect_url"), "https://www.example.com")
        HttpCheck(DirectiveCommand("dom_element"), {})
        HttpCheck(DirectiveCommand("count_instances"), {"model": Profile, "n": 1})
        with self.assertRaises(ValueError):
            HttpCheck(DirectiveCommand("status_code"), "200")
        with self.assertRaises(ValueError):
            HttpCheck(DirectiveCommand("redirect_url"), 0)
        with self.assertRaises(ValueError):
            HttpCheck(DirectiveCommand("dom_element"), 0)
        with self.assertRaises(ValueError):
            HttpCheck(DirectiveCommand("count_instances"), {"model": NotAModel, "n": 1})


class TestHttpTake(unittest.TestCase):

    def test(self):
        take = HttpTake(
            "GET",
            "https://www.example.com",
            [HttpDirective(DirectiveCommand("status_code"), 200)],
            [],
            {},
            {},
        )
        self.assertEqual(take.method, HTTPMethod.GET)


#################
# MANIFEST PARSER
#################


class TestManifestParser(unittest.TestCase):

    def test_parse_formatted_dict(self):
        d = {
            "set_up_test_data": [],
            "set_up": [],
            "cases": {},
            "scenes": [],
            "manifest_origin": "origin",
        }
        ManifestParser.parse_formatted_dict(d)
        d.pop("cases")
        with self.assertRaises(KeyError):
            ManifestParser.parse_formatted_dict(d)

    def test_validate_dict(self):
        manifest_base_dict = {
            "set_up_test_data": object(),
            "set_up": object(),
            "cases": object(),
            "scenes": object(),
            "manifest_origin": "origin",
        }

        # Unknown key
        manifest = manifest_base_dict | {"unknown": object()}
        with self.assertRaises(ValueError):

            ManifestParser.validate_dict(manifest)

        # Both case and cases
        manifest = manifest_base_dict | {"case": object()}
        with self.assertRaisesRegex(
            ValueError,
            r"Both `case` and `cases` keys are present at top level\.$",
        ):
            ManifestParser.validate_dict(manifest)

        # Neither case or cases
        manifest = manifest_base_dict.copy()
        manifest.pop("cases")
        with self.assertRaisesRegex(
            ValueError,
            r"Neither `case` and `cases` keys are present at top level\.$",
        ):
            ManifestParser.validate_dict(manifest)

        # Both scene and scenes
        manifest = manifest_base_dict | {"scene": object()}
        with self.assertRaisesRegex(
            ValueError,
            r"Both `scene` and `scenes` keys are present at top level\.$",
        ):
            ManifestParser.validate_dict(manifest)

        # Neither scene or scenes
        manifest = manifest_base_dict.copy()
        manifest.pop("scenes")
        with self.assertRaisesRegex(
            ValueError,
            r"Neither `scene` and `scenes` keys are present at top level\.$",
        ):
            ManifestParser.validate_dict(manifest)

        # Success all fields
        manifest = manifest_base_dict
        ManifestParser.validate_dict(manifest)

        # Success no optional field
        manifest = manifest_base_dict.copy()
        manifest.pop("set_up")
        manifest.pop("set_up_test_data")
        ManifestParser.validate_dict(manifest)

    def test_format_dict(self):
        scene_1 = object()
        scene_2 = object()
        case_1 = object()
        case_2 = object()
        manifest = {
            "cases": [case_1, case_2],
            "scenes": [scene_1, scene_2],
            "manifest_origin": "origin",
        }
        ManifestParser.validate_dict(manifest)
        manifest = ManifestParser.format_dict(manifest)
        self.assertDictEqual(
            manifest,
            {
                "cases": [case_1, case_2],
                "scenes": [scene_1, scene_2],
                "manifest_origin": "origin",
                "set_up_test_data": [],
                "set_up": [],
            },
        )
        manifest = {
            "case": case_1,
            "scene": scene_1,
            "manifest_origin": "origin",
            "set_up_test_data": ["a", "b"],
            "set_up": ["c", "d"],
        }
        ManifestParser.validate_dict(manifest)
        manifest = ManifestParser.format_dict(manifest)
        self.assertDictEqual(
            manifest,
            {
                "cases": {"UNIQUE_CASE": case_1},
                "scenes": [scene_1],
                "manifest_origin": "origin",
                "set_up_test_data": ["a", "b"],
                "set_up": ["c", "d"],
            },
        )

    def test__format_dict_scenes(self):

        scene_1 = object()
        scene_2 = object()
        manifest_base_dict = {
            "cases": object(),
            "manifest_origin": "origin",
        }

        # Single scene
        manifest = manifest_base_dict | {"scene": scene_1}
        ManifestParser.validate_dict(manifest)
        scenes = ManifestParser._format_dict_scenes(manifest)
        self.assertListEqual(scenes, [scene_1])

        # Single scene in scenes
        manifest = manifest_base_dict | {"scenes": [scene_1]}
        ManifestParser.validate_dict(manifest)
        scenes = ManifestParser._format_dict_scenes(manifest)
        self.assertListEqual(scenes, [scene_1])

        # Scenes
        manifest = manifest_base_dict | {"scenes": [scene_1, scene_2]}
        ManifestParser.validate_dict(manifest)
        scenes = ManifestParser._format_dict_scenes(manifest)
        self.assertListEqual(scenes, [scene_1, scene_2])

    def test__format_dict_cases(self):

        case_1 = object()
        case_2 = object()
        manifest_base_dict = {
            "scenes": object(),
            "manifest_origin": "origin",
        }

        # Single case
        manifest = manifest_base_dict | {"case": case_1}
        ManifestParser.validate_dict(manifest)
        cases = ManifestParser._format_dict_cases(manifest)
        self.assertDictEqual(cases, {"UNIQUE_CASE": case_1})

        # Single case in cases
        manifest = manifest_base_dict | {"cases": {"case_id": case_1}}
        ManifestParser.validate_yaml(manifest)
        cases = ManifestParser._format_dict_cases(manifest)
        self.assertDictEqual(cases, {"case_id": case_1})

        # Cases
        manifest = manifest_base_dict | {"cases": {"case_1": case_1, "case_2": case_2}}
        ManifestParser.validate_dict(manifest)
        cases = ManifestParser._format_dict_cases(manifest)
        self.assertDictEqual(cases, {"case_1": case_1, "case_2": case_2})

    def test_parse_dict(self):
        d = {
            "case": {},
            "scene": {
                "method": "GET",
                "url": "https://www.example.com",
                "directives": [{"status_code": 200}],
            },
            "manifest_origin": "origin",
            "set_up_test_data": ["reset_db"],
            "set_up": ["create_testuser"],
        }
        ManifestParser.parse_dict(d)
        d.pop("case")
        with self.assertRaises(ValueError):
            ManifestParser.parse_dict(d)

    def test_validate_yaml(self):

        # wrong type
        manifest = []
        with self.assertRaisesRegex(
            TypeError, r"^Manifest need to be a dict not a '<class 'list'>'$"
        ):
            ManifestParser.validate_yaml(manifest)

        # success
        manifest = {
            "variables": object(),
            "cases": object(),
            "scenes": object(),
            "manifest_origin": "origin",
        }
        ManifestParser.validate_yaml(manifest)


##############
# TAKE BUILDER
##############


# class TestTakeBuilder(unittest.TestCase):

#     def setUp(self):
#         super().setUp()

#         case_dict = {
#             "item_id": {
#                 "data": {
#                     "a": object(),
#                     "b": object(),
#                 },
#                 "url_parameters": {"a": object()},
#                 "checks": {
#                     # not object as we check the type of this argument
#                     "a": "something",
#                     "b": object(),
#                 },  # not object as we check the type of this argument
#             }
#         }

#         self.case = Case.from_id_and_dict("case_id", case_dict)

#         self.scene_base_dict = {
#             "method": "GET",
#             "url": "https://www.example.com",
#             "directives": [{"status_code": 200}],
#         }

#     def test_regex(self):

#         self.assertRegex("item", TakeBuilder.regex_field)
#         self.assertRegex("item_with_underscore", TakeBuilder.regex_field)
#         self.assertRegex("item:field", TakeBuilder.regex_field)
#         self.assertRegex("item:field_with_underscore", TakeBuilder.regex_field)
#         self.assertNotRegex("item_Uppercase", TakeBuilder.regex_field)
#         self.assertNotRegex("item_0", TakeBuilder.regex_field)
#         self.assertNotRegex("item-with-hyphen", TakeBuilder.regex_field)
#         self.assertNotRegex("item:field_Uppercase", TakeBuilder.regex_field)
#         self.assertNotRegex("item:field_0", TakeBuilder.regex_field)
#         self.assertNotRegex("item:field-with-hyphen", TakeBuilder.regex_field)

#     def test_substitute_from_case(self):

#         # error
#         with self.assertRaisesRegex(
#             ValueError, r"^Invalid field representation 'item_id!a'$"
#         ):
#             TakeBuilder.substitute_from_case("item_id!a", HttpTarget.DATA, self.case)
#         # Non existing field
#         with self.assertRaises(KeyError):
#             TakeBuilder.substitute_from_case("itemi_id:c", HttpTarget.DATA, self.case)

#         # Whole item
#         substitution = TakeBuilder.substitute_from_case(
#             "item_id", HttpTarget.DATA, self.case
#         )
#         self.assertDictEqual(substitution, self.case["item_id"][HttpTarget.DATA])

#         # Single field
#         substitution = TakeBuilder.substitute_from_case(
#             "item_id:a", HttpTarget.DATA, self.case
#         )
#         self.assertEqual(substitution, self.case["item_id"][HttpTarget.DATA]["a"])

#     def test_substitute_recursively(self):

#         substitution_field = {"__from_case__": "item_id"}
#         scene = HttpScene.from_dict(
#             self.scene_base_dict
#             | {
#                 "data": substitution_field,
#                 "url_parameters": substitution_field,
#                 "query_parameters": substitution_field,
#             }
#         )
#         data = TakeBuilder.substitute_recursively(
#             scene.data, HttpTarget.DATA, self.case
#         )
#         self.assertDictEqual(data, self.case["item_id"][HttpTarget.DATA])
#         url_parameters = TakeBuilder.substitute_recursively(
#             scene.url_parameters, HttpTarget.URL_PARAMETERS, self.case
#         )
#         self.assertDictEqual(
#             url_parameters, self.case["item_id"][HttpTarget.URL_PARAMETERS]
#         )
#         checks = TakeBuilder.substitute_recursively(
#             scene.url_parameters, HttpTarget.CHECKS, self.case
#         )
#         self.assertDictEqual(checks, self.case["item_id"][HttpTarget.CHECKS])

#     def test_shoot(self):
#         scene = HttpScene.from_dict(
#             self.scene_base_dict
#             | {"data": {"a": {"__from_case__": "item_id:a"}}}
#             | {"url_parameters": {"key": {"__from_case__": "item_id:a"}}}
#             | {
#                 "directives": [
#                     {
#                         "dom_element": {
#                             "find": {"id": {"__from_case__": "item_id:b"}},
#                             "attribute": {
#                                 "name": "name",
#                                 "value": {"__from_case__": "item_id:a"},
#                             },
#                         }
#                     }
#                 ]
#             }
#         )
#         take = TakeBuilder.shoot(scene, self.case)
#         self.assertEqual(
#             take,
#             HttpTake(
#                 method=self.scene_base_dict["method"],
#                 url=self.scene_base_dict["url"],
#                 data={"a": self.case["item_id"][HttpTarget.DATA]["a"]},
#                 url_parameters={
#                     "key": self.case["item_id"][HttpTarget.URL_PARAMETERS]["a"]
#                 },
#                 query_parameters={},
#                 checks=[
#                     HttpCheck(
#                         instruction=DirectiveCommand.DOM_ELEMENT,
#                         args={
#                             "find": {
#                                 "id": self.case["item_id"][HttpTarget.CHECKS]["b"],
#                             },
#                             "attribute": {
#                                 "name": "name",
#                                 "value": self.case["item_id"][HttpTarget.CHECKS]["a"],
#                             },
#                         },
#                     )
#                 ],
#             ),
#         )


#####################
# SET UP INSTRUCTIONS
#####################


class TestSetUpHandler(unittest.TestCase):

    def setUp(self):
        # Maybe we don't need explicit here
        # But on the other hand I did not want to use reset_db
        # As the purpose of this class is to test it
        # Order of Profile then UserCustom matters
        Profile.objects.all().delete()
        UserCustom.objects.all().delete()
        School.objects.all().delete()
        UserInteraction.objects.all().delete()
        UserInteractionDetail.objects.all().delete()
        StripeSubscription.objects.all().delete()
        StripeCustomer.objects.all().delete()

    # def test_load_highschools(self):
    #     load_higshcools()  # this takes a lot of time to run
    #     schools = School.objects.all()
    #     self.assertEqual(len(schools), )

    def test_create_testuser(self):
        SetUpHandler.create_testuser(
            user_email="email@mail.com",
            first_name="Harry",
            last_name="Potter",
            password="password",
            is_active=True,
            birth_date_year=1980,
            birth_date_month=7,
            birth_date_day=31,
            class_at_school="premiere",
            contract=0,
        )
        profiles = Profile.objects.all()
        users = UserCustom.objects.all()
        self.assertEqual(len(profiles), 1)
        self.assertEqual(len(users), 1)
        self.assertFalse(users[0].is_superuser)

    # def test_create_testuser_kwargs(self):
    #     SetUpHandler.create_testuser(user_email="another_email@mail.com")
    #     profiles = Profile.objects.all()
    #     users = UserCustom.objects.all()
    #     self.assertEqual(len(profiles), 1)
    #     self.assertEqual(len(users), 1)
    #     self.assertFalse(users[0].is_superuser)
    #     self.assertEqual(users[0].username, "another_email@mail.com")

    def test_create_superuser(self):
        SetUpHandler.create_superuser()
        profiles = Profile.objects.all()
        users = UserCustom.objects.all()
        self.assertEqual(len(profiles), 1)
        self.assertEqual(len(users), 1)
        self.assertTrue(users[0].is_superuser)

    def test_reset_db(self):
        # As it is tested separately, we assume create_testuser is working
        # allowing to check the created instance is removed
        SetUpHandler.create_testuser(
            user_email="email@mail.com",
            first_name="Harry",
            last_name="Potter",
            password="password",
            is_active=True,
            birth_date_year=1980,
            birth_date_month=7,
            birth_date_day=31,
            class_at_school="premiere",
            contract=0,
        )
        SetUpHandler.reset_db()
        app_config = apps.get_app_config("app")
        for model in app_config.get_models():
            instances = model.objects.all()
            self.assertEqual(
                len(instances), 0, f"{model.__name__} was not reset properly"
            )


########
# CHECKS
########


class TestHttpChecker(TestCaseOfDjangoTestCase):

    def test_check_status_code(self):

        response = HttpResponse()
        response.status_code = 200

        def test_pass(django_testcase):
            HttpChecker.check_status_code(django_testcase, response, 200)

        def test_fail(django_testcase):
            HttpChecker.check_status_code(django_testcase, response, 400)

        self.django_testcase.test_pass = test_pass
        self.django_testcase.test_fail = test_fail

        self.assertTestPasses(self.django_testcase("test_pass"))
        self.assertTestFails(self.django_testcase("test_fail"))

    def test_check_redirect_url(self):
        response = HttpResponseRedirect(redirect_to="somewhere")

        def test_pass(django_testcase):
            HttpChecker.check_redirect_url(django_testcase, response, "somewhere")

        def test_fail(django_testcase):
            HttpChecker.check_redirect_url(django_testcase, response, "elsewhere")

        self.django_testcase.test_pass = test_pass
        self.django_testcase.test_fail = test_fail

        self.assertTestPasses(self.django_testcase("test_pass"))
        self.assertTestFails(self.django_testcase("test_fail"))

    def test_check_count_instances(self):
        SetUpHandler.reset_db()  # As is it tested in TestSetUpInstructions we assume it is working
        response = HttpResponse()

        def test_pass(django_testcase):
            HttpChecker.check_count_instances(
                django_testcase, response, {"model": Profile, "n": 0}
            )

        def test_fail(django_testcase):
            HttpChecker.check_count_instances(
                django_testcase, response, {"model": Profile, "n": 1}
            )

        # def test_error(django_testcase):
        #     class UndefinedModel:
        #         pass

        #     HttpChecker.check_count_instances(
        #         django_testcase, response, {"model": UndefinedModel, "n": 0}
        #     )

        self.django_testcase.test_pass = test_pass
        self.django_testcase.test_fail = test_fail
        # self.django_testcase.test_error = test_error

        self.assertTestPasses(self.django_testcase("test_pass"))
        self.assertTestFails(self.django_testcase("test_fail"))
        # This actually raises an AttributeError as now data validation is
        # Performed by the HttpTake dataclasses
        # self.assertTestRaises(self.django_testcase("test_error"), LookupError)

    def test_check_dom_element(self):

        def test_pass_find_by_id(django_testcase):
            response = HttpResponse()
            response.content = '<div id="target">Pass</div>'
            HttpChecker.check_dom_element(
                django_testcase, response, {DomArgument.FIND: {"id": "target"}}
            )

        def test_pass_find_by_class(django_testcase):
            response = HttpResponse()
            response.content = '<div class="target">Pass</div>'
            HttpChecker.check_dom_element(
                django_testcase, response, {DomArgument.FIND: {"class": "target"}}
            )

        def test_pass_text(django_testcase):
            response = HttpResponse()
            response.content = '<div id="target">Pass</div>'
            HttpChecker.check_dom_element(
                django_testcase,
                response,
                {DomArgument.FIND: {"id": "target"}, DomArgument.TEXT: "Pass"},
            )

        def test_pass_attribute(django_testcase):
            response = HttpResponse()
            response.content = '<div id="target" class="something">Pass</div>'
            HttpChecker.check_dom_element(
                django_testcase,
                response,
                {
                    DomArgument.FIND: {"id": "target"},
                    DomArgument.ATTRIBUTE: {"name": "class", "value": ["something"]},
                },
            )

        def test_pass_find_all(django_testcase):
            response = HttpResponse()
            response.content = (
                '<div class="something">Pass</div> <div class="something">Pass</div>'
            )
            HttpChecker.check_dom_element(
                django_testcase,
                response,
                {
                    DomArgument.FIND_ALL: {"class": "something"},
                    DomArgument.COUNT: 2,
                },
            )

        def test_pass_scope(django_testcase):
            response = HttpResponse()
            response.content = '<div id="scope"> <div class="something">In</div> <div class="something">In</div> </div> <div class="something">'
            HttpChecker.check_dom_element(
                django_testcase,
                response,
                {
                    DomArgument.SCOPE: {"id": "scope"},
                    DomArgument.FIND_ALL: {"class": "something"},
                    DomArgument.COUNT: 2,
                },
            )

        def test_fail_1(django_testcase):
            response = HttpResponse()
            response.content = '<div id="target">Pass</div>'
            HttpChecker.check_dom_element(
                django_testcase, response, {DomArgument.FIND: {"id": "another_target"}}
            )

        def test_fail_2(django_testcase):
            response = HttpResponse()
            response.content = '<div id="target">Pass</div>'
            HttpChecker.check_dom_element(
                django_testcase,
                response,
                {DomArgument.FIND: {"id": "target"}, DomArgument.TEXT: "Fail"},
            )

        def test_fail_3(django_testcase):
            response = HttpResponse()
            response.content = '<div id="target" class="something">Pass</div>'
            HttpChecker.check_dom_element(
                django_testcase,
                response,
                {
                    DomArgument.FIND: {"id": "target"},
                    DomArgument.ATTRIBUTE: {
                        "name": "class",
                        "value": ["something_else"],
                    },
                },
            )

        def test_error_1(django_testcase):
            response = HttpResponse()
            response.content = '<div id="target" class="something">Pass</div>'
            HttpChecker.check_dom_element(
                django_testcase,
                response,
                {},
            )

        self.django_testcase.test_pass_find_by_id = test_pass_find_by_id
        self.django_testcase.test_pass_find_by_class = test_pass_find_by_class
        self.django_testcase.test_pass_text = test_pass_text
        self.django_testcase.test_pass_attribute = test_pass_attribute
        self.django_testcase.test_pass_find_all = test_pass_find_all
        self.django_testcase.test_pass_scope = test_pass_scope
        self.django_testcase.test_fail_1 = test_fail_1
        self.django_testcase.test_fail_2 = test_fail_2
        self.django_testcase.test_fail_3 = test_fail_3
        self.django_testcase.test_error_1 = test_error_1

        self.assertTestPasses(self.django_testcase("test_pass_find_by_id"))
        self.assertTestPasses(self.django_testcase("test_pass_find_by_class"))
        self.assertTestPasses(self.django_testcase("test_pass_attribute"))
        self.assertTestPasses(self.django_testcase("test_pass_text"))
        self.assertTestPasses(self.django_testcase("test_pass_find_all"))
        self.assertTestPasses(self.django_testcase("test_pass_scope"))
        self.assertTestFails(self.django_testcase("test_fail_1"))
        self.assertTestFails(self.django_testcase("test_fail_2"))
        self.assertTestFails(self.django_testcase("test_fail_3"))
        self.assertTestRaises(self.django_testcase("test_error_1"), ValueError)


################
# METHOD BUILDER
################


class TestMethodBuilder(TestCaseOfDjangoTestCase):

    exec_order = []

    def test_execution_order(self):
        """
        We check the setUpTestData function is executed before the tests and only once
        As opposed to setUp
        Note that currently, there is only one single test by TestCase (ie by Take)
        But we could therefore easily go beyond
        """

        ## Reset class attribute
        TestMethodBuilder.exec_order = []

        take = HttpTake(HTTPMethod.GET, "https://www.example.com", [], {}, {}, {})

        def watch(func):

            def wrapper(*args, **kwargs):

                # print("beginning with exec_order")
                # print(TestMethodBuilder.exec_order)
                # print("trying this func.__name__")
                # print(func.__name__)
                # print(type(func))
                if type(func) is classmethod:

                    # print("is_class_method")
                    # otherwise the call fails
                    method = func.__get__(
                        self.django_testcase, type(self.django_testcase)
                    )
                    x = method(*args, **kwargs)
                else:
                    # print("is_not_class_method")
                    # print("args")
                    # print(args)
                    # print("kwargs")
                    # print(kwargs)
                    x = func(*args, **kwargs)
                #     print("execution_succedded")

                # print(x)
                # print("exec order before append")
                # print(TestMethodBuilder.exec_order)
                TestMethodBuilder.exec_order.append(func.__name__)
                # print("ending with exec_order")
                # print(TestMethodBuilder.exec_order)

                # print("\n\n")
                return x

            return wrapper

        self.django_testcase.setUpTestData = watch(
            MethodBuilder.build_setUpTestData([])
        )
        self.django_testcase.setUp = watch(MethodBuilder.build_setUp([]))

        test_1 = MethodBuilder.build_test_from_take(take)
        test_1.__name__ = "test_1"
        test_2 = MethodBuilder.build_test_from_take(take)
        test_2.__name__ = "test_2"

        self.django_testcase.test_1 = watch(test_1)
        self.django_testcase.test_2 = watch(test_2)

        self.run_django_testcase()

        # print("self.django_testcase.test_1", self.django_testcase.test_1)
        # print("self.django_testcase.test_2", self.django_testcase.test_2)

        self.assertListEqual(
            # self.django_testcase.execution_order,
            TestMethodBuilder.exec_order,
            ["setUpTestData", "setUp", "test_1", "setUp", "test_2"],
        )

    def test_persistency_test_data(self):
        # Check the persistency of data created during tests
        self.django_testcase.setUpTestData = MethodBuilder.build_setUpTestData(
            [
                SetUpInstruction(SetUpCommand.RESET_DB),
                SetUpInstruction(
                    SetUpCommand.CREATE_TESTUSER,
                    {
                        "user_email": "email@mail.com",
                        "first_name": "Harry",
                        "last_name": "Potter",
                        "password": "password",
                        "is_active": True,
                        "birth_date_year": 1980,
                        "birth_date_month": 7,
                        "birth_date_day": 31,
                        "class_at_school": "premiere",
                        "contract": 0,
                    },
                ),
            ]
        )

        def test_1(django_testcase):
            # NOTE: the assertion is done on the unittest.TestCase and not the django.TestCase
            profiles = Profile.objects.all()
            self.assertEqual(len(profiles), 1)

        def test_2(django_testcase):
            # NOTE: the assertion is done on the unittest.TestCase and not the django.TestCase
            SetUpHandler.create_testuser(
                user_email="another_email@mail.com",
                first_name="Harry",
                last_name="Potter",
                password="password",
                is_active=True,
                birth_date_year=1980,
                birth_date_month=7,
                birth_date_day=31,
                class_at_school="premiere",
                contract=0,
            )
            profiles = Profile.objects.all()
            self.assertEqual(len(profiles), 2)

        self.django_testcase.test_1 = test_1
        self.django_testcase.test_2 = test_2

        self.assertTestPasses(self.django_testcase("test_1"))
        self.assertTestPasses(self.django_testcase("test_2"))


###########################################
# TEST CASE LOGGER
###########################################

# Logging config should probably be somewhere else

# class CustomHandler(logging.Handler):

#     def emit(self: logging.Handler, record):
#         msg = self.format(record)
#         print(msg)


# @classmethod
# def setUpClass(cls):
#     """Set up logging once for all tests in this class."""

#     format_log = "[%(asctime)s.%(msecs)03d] [%(levelname)s] %(message)-1.250s"
#     datefmt_ = "%Y-%m-%d %H:%M:%S"
#     cls.logger = logging.getLogger(cls.__name__)
#     cls.formatter = logging.Formatter(fmt=format_log, datefmt=datefmt_)
#     cls.handler = logging.StreamHandler()
#     cls.handler.setFormatter(cls.formatter)
#     cls.handler.setLevel(logging.DEBUG)
#     cls.logger.addHandler(cls.handler)
#     cls.logger.setLevel(logging.DEBUG)

#     # This is a bit rough/ugly
#     cls.manager_disable = cls.logger.manager.disable
#     cls.logger.manager.disable = logging.NOTSET
#     cls.logger.propagate = False

# @classmethod
# def tearDownClass(cls):
#     # Reset initial value
#     cls.logger.manager.disable = cls.manager_disable


###########################################
# ATTEMPT TO USE HTTPBIN (probably useless)
###########################################

# response = requests.get("https://httpbin.org/status/200")
# response = self._convert_django_to_response(response)

# @staticmethod
# def _convert_django_to_response(
#     requests_response: requests.Response,
# ) -> django.http.HttpResponse:
#     #Alternatively we could just mock the http response

#     django_response = django.http.HttpResponse()
#     # Set the content of the response
#     django_response.content = requests_response.content
#     # Set the status code of the response
#     django_response.status_code = requests_response.status_code
#     # Copy relevant headers from the requests response to the Django response
#     for key, value in requests_response.headers.items():
#         # Exclude headers that should not be copied or might cause issues
#         if key.lower() not in ["content-length", "transfer-encoding", "connection"]:
#             django_response[key] = value
#     return django_response


# class CustomDjangoTestCase(django.test.TestCase):

#     execution_order = []

#     @classmethod
#     def watch_execution(cls):

#         def decorator(method):
#             # Check if the method is a classmethod or staticmethod
#             if isinstance(method, classmethod) or isinstance(method, staticmethod):
#                 original_func = method.__func__
#             else:
#                 original_func = method

#             @wraps(original_func)
#             def wrapper(*args, **kwargs):
#                 # Add method name to the execution_order list
#                 cls.execution_order.append(original_func.__name__)
#                 return original_func(*args, **kwargs)

#             if isinstance(method, classmethod):
#                 return classmethod(wrapper)
#             elif isinstance(method, staticmethod):
#                 # For a static method, rewrap as staticmethod
#                 return staticmethod(wrapper)
#             return wrapper

#         return decorator

#     @classmethod
#     def monitor_execution_order(cls):

#         # Define which methods to monitor
#         def is_watched_method(method_name):
#             return method_name.startswith("test_") or method_name in [
#                 "setUp",
#                 "setUpTestData",
#             ]

#         for attr_name in dir(cls):
#             if is_watched_method(attr_name):
#                 method = getattr(cls, attr_name)
#                 setattr(cls, attr_name, cls.watch_execution()(method))

#     @classmethod
#     def update_monitoring(cls):
#         cls.monitor_execution_order()
